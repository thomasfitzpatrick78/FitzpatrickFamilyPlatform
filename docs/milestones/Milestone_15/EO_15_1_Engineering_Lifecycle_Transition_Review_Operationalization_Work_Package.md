...
2. verify a clean baseline, the exact baseline approved by the published Architecture Gatekeeper decision, or an `Expected Generated Evidence` baseline under the implemented and published policy;
...
| Version | Description |
|---------|-------------|
| 1.1 | Published the Architecture Gatekeeper-approved authority amendment that makes fail-closed generated-evidence baseline classification the first EO-15.1 implementation deliverable while preserving all product, architecture, release, activation, deployment, and live-work gates. |
