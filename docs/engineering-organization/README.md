# Engineering Organization Foundation

**Document Version:** 1.6

**Status:** Active

**Milestone:** EO-13.0

---

## Purpose

This governed documentation area establishes the AI Engineering Organization as a first-class product and operating model for the Fitzpatrick Family Platform Engineering Organization.

---

## Foundation

The AI-operated Engineering Organization is the primary governed operating capability. The Fitzpatrick Family Platform is its reference implementation and the shared technical foundation. The Fitzpatrick Family Financial Assistant is the flagship customer-facing application enabled by the organization and Platform.

The Engineering Organization operates across three layers:

| Layer | Responsibility |
|-------|----------------|
| Engineering Organization | Product strategy, governance, roles, delivery model, memory, and operating practices. |
| Shared Platform | Shared governed capabilities, infrastructure registry, automation, observability, and reusable engineering foundation. |
| Customer-Facing Applications | User-facing products and experiences that deliver direct household value, led by the Fitzpatrick Family Financial Assistant. |

---

## Artifacts

- [Engineering Organization Vision](Engineering_Organization_Vision.md)
- [Engineering Organization Manifesto](Engineering_Organization_Manifesto.md)
- [Portfolio Strategy](Portfolio_Strategy.md)
- [AI Role Catalog](AI_Role_Catalog.md)
- [Human Role Catalog](Human_Role_Catalog.md)
- [Engineering Capability Model](Engineering_Capability_Model.md)
- [Engineering Organization Capability Maturity Model](Engineering_Organization_Capability_Maturity_Model.md)
- [Engineering Organization Roadmap](Engineering_Organization_Roadmap.md)
- [Engineering Organization Backlog](Engineering_Organization_Backlog.md)
- [Parallel Workstream Delivery Model](Parallel_Workstream_Delivery_Model.md)
- [Engineering Workspace Model](Engineering_Workspace_Model.md)
- [Organizational Impact Assessment Template](Organizational_Impact_Assessment_Template.md)
- [Architecture Pattern Library](Architecture_Pattern_Library.md)
- [Engineering Memory Concept](Engineering_Memory_Concept.md)
- [Customer Environment Cutover Checklist](Customer_Environment_Cutover_Checklist.md)
- [Engineering Metrics v2](Engineering_Metrics_v2.md)
- [Execution Capability Usage](Execution_Capability_Usage.md)
- [Governed Automation Framework](Governed_Automation_Framework.md)
- [Governed Automation Framework Usage](Governed_Automation_Framework_Usage.md)
- [AI Collaboration Governance](ai-collaboration/README.md)

---

## Related Documents

- [Permanent Project Operating Model](../governance/Permanent_Project_Operating_Model.md)
- [Engineering Principles](../governance/Engineering_Principles.md)
- [Engineering Lifecycle](../governance/Engineering_Lifecycle.md)
- [Shared Engineering Strategy](../portfolio/Shared_Engineering_Strategy.md)
- [Product Portfolio](../governance/Product_Portfolio.md)

---

## Revision History

| Version | Description |
|---------|-------------|
| 1.6 | Linked the EO-14.1A Execution Capability and EO-14.4A governed orchestration implementation and usage artifacts. |
| 1.5 | Recorded EO-14.8E readiness observability integration and linked Engineering Metrics v2. |
| 1.4 | Recorded EO-14.8D AI Session Readiness Validator implementation in the AI Collaboration Governance area. |
| 1.3 | Added AI Collaboration Governance specification package and framework index. |
| 1.2 | Added reusable Customer Environment Cutover Checklist. |
| 1.1 | Added EO-13.1 governed capability language, manifesto, maturity model, and three-pillar terminology. |
| 1.0 | Initial Engineering Organization foundation artifact index. |
