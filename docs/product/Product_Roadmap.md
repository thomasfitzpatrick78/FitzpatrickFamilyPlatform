...
- Preserve the Architecture Gatekeeper-approved, accepted, and published transport-free privileged-proxy source while retaining ADR-012 `Implemented: No`, the authenticated non-Docker interface, threat model, runtime/supply-chain controls, security tests, and every socket, artifact, deployment, observation, consumer, and activation gate.
...
- Production Provider Adapter Architecture, security design, repository foundation, formal proxy security review, transport-incapable proxy foundation, repository-only deployment configuration foundation, purpose-built implementation architecture/acceptance package, and Architecture Gatekeeper-approved and accepted transport-free standard-library Go source foundation are published as named PLAT-14.1A prerequisites. Socket-capable implementation, artifact acceptance, deployment, eligible-target, observation, consumer, and activation gates remain separate; no socket-capable implementation is authorized.
