...
- PLAT-15.1A is approved and published under AB-011 as the future repository-only socket-capable privileged-proxy source work package. Its supported-Linux prerequisite direction is a task-specific disposable native ARM64 Linux VM with no guest network adapter, separate static environment and per-run execution manifests, named host authorization for every host action, preparation-only VM-local administrative privilege, and a bounded non-administrative implementation/validation identity. Direction approval does not select a virtualization mechanism or image and does not create or approve the preparation work package. The environment is `Not Created`; repository implementation is `Not Started` and blocked. The direction does not reopen the closed transport architecture or change ADR-012, AB-012, the Production Provider Adapter, or any artifact, deployment, daemon-interaction, observation, consumer, recurrence, activation, release, or live gate.
