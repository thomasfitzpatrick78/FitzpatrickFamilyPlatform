...
- PLAT-15.1A is approved and published under AB-011 as the future repository-only socket-capable privileged-proxy source work package. It is `Ready for Repository Implementation; Not Started`, does not reopen the closed transport architecture, and does not change ADR-012, AB-012, the Production Provider Adapter, or any artifact, deployment, daemon-interaction, observation, consumer, recurrence, activation, release, or live gate.
